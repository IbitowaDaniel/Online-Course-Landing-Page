...
const Pricing = () => {
  ...
  return (
    <section id="pricing" className="boxWidth paddingX paddingY">
      <SectionHeader
        heading="Pricing Plan"
        headingSecondary="Choose your pricing policy"
      />
                  ...
                  >
                    {c.map((item) => (
                      <div key={item.id} className='relative rounded-2xl border mt-6 mb-8 px-6 pt-10 pb-14 sm:pl-8 mx-auto max-w-sm xs:mx-auto xs:max-w-sm md:max-w-xs lg:max-w-[300px] xl:max-w-[350px]'>
                        ...
                        {item.points.map((c, i) => (
                          <div key={i}>
                            <ul className='list-none -ml-2 flex gap-x-2'><button className="animate-button w-6 h-12 text-[#25CB9E] -ml-[1px] text-2xl flex-shrink-0" aria-label="list icon">{c.icon}</button>
                        ...
                        <div className='flex flex-col justify-center items-center'>
                          ...
                          <button className='bg-primaryLight py-3 px-5 text-primary hover:text-white hover:bg-primary transition duration-500 ease-in-out text-sm font-semibold mt-4 rounded-[45px] animate-button border'>{item.buttonText}</button>
